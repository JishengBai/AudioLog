...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
                in_chans=1, num_classes=527, asc_num_classes=5,
        ...
        self.num_asc_classes = asc_num_classes
        self.num_heads = num_heads
        ...
        if self.config.enable_tscam:
            ...
            self.ts_pool = nn.AdaptiveAvgPool2d((1, 32))
            
            self.tscam_conv_asc = nn.Conv2d(
                in_channels = self.num_features,
                out_channels = 128,
                kernel_size = (1,32),
                padding = (0,0)
            ...
            self.bn_asc = nn.BatchNorm2d(128)
            self.bn_sed = nn.BatchNorm2d(128)

            self.sed_proj = nn.Linear(32, 1)
            self.asc_head = nn.Linear(128, self.num_asc_classes)
            self.sed_head = nn.Linear(128, self.num_classes)
        else:
    ...
    def forward_features(self, x):
        ...
        if self.config.enable_tscam:
            ...
            # x_sed = torch.mean(x, 2)
            # x_sed = torch.unsqueeze(x_sed, 2)
            x_sed = self.tscam_conv(x) 
            x_sed = Fu.relu(self.bn_sed(x_sed))
            x_sed = torch.flatten(x_sed, 2)  
            e_sed = self.sed_proj(x_sed)
            e_sed = torch.flatten(e_sed, 1)
            e_sed = Fu.normalize(e_sed, dim=-1)
            
            # x_asc, _ = torch.max(x, 2)
            # x_asc = torch.unsqueeze(x_asc, 2)
            x_asc = self.tscam_conv_asc(x)
            x_asc = Fu.relu(self.bn_asc(x_asc))
            e_asc = torch.flatten(x_asc, 1)
            e_asc = Fu.normalize(e_asc, dim=-1)
            
            x_asc = self.asc_head(e_asc)
            x_sed = x_sed.permute(0,2,1)
            fpx = self.sed_head(x_sed)
            fpx = fpx.permute(0,2,1)

            if self.config.htsat_attn_heatmap:
                fpx = interpolate(torch.sigmoid(fpx).permute(0,2,1).contiguous() * attn, 8 * self.patch_stride[1]) 
            else: 
                fpx = interpolate(torch.sigmoid(fpx).permute(0,2,1).contiguous(), 2* self.patch_stride[1]) 

            if self.config.loss_type == "clip_ce":
                output_dict = {
                    'framewise_output': fpx, # already sigmoided
                    'clipwise_output': x_asc,
                    'latent_output': [e_asc, e_sed],
...
if __name__ == '__main__':
    import config
    import torch
    model = HTSAT_Swin_Transformer(num_classes=25, asc_num_classes=5, config=config)
    # print(model)
    inputs = torch.rand((2, 320000))
    out = model(inputs)
    asc_out = out['clipwise_output']   
    sed_out = out['framewise_output']   
    print("asc:", asc_out.shape, 'sed:',sed_out.shape)