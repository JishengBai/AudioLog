...
class DCASE23T4B_Dataset(object):
    ...
    def get_data(self):
        ...
        for file_path in self.file_list:
            ...
            if len(file)<=10*sr:
                file = np.concatenate((file, np.zeros(10*sr-len(file))))

            data.append(file)
        for label_path in self.label_list:
            lb = np.load(label_path, allow_pickle=True)
            ### T4B
            lb = np.concatenate((lb[:, :11], lb[:, 22:]), axis=1)
            ###
            label.append(lb)
    ...
    def get_tensordataset(self):
        ...
        if self.is_train:
            loader = DataLoader(dataset=dataset, batch_size=self.batch_size, shuffle=True, num_workers=8, 
                                pin_memory=True)
        else:
            loader = DataLoader(dataset=dataset, batch_size=self.batch_size, shuffle=False, num_workers=8, 
                                pin_memory=True)
        return loader
...
class Test_Dataset(object):
    ...
    def get_data(self):
        ...
        for file_path in self.file_list:
            seg_arr = []
            file, sr = librosa.load(file_path, sr=self.sr, res_type='kaiser_fast')
