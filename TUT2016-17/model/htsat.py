...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
        ...
        if self.config.enable_tscam:
            ...
            self.asc_head = nn.Linear(num_classes, self.num_asc_classes)
        else:
    ...
    def forward_features(self, x):
        ...
        if self.config.enable_tscam:
            ...
            if self.config.htsat_attn_heatmap:
                ...
                fpx = interpolate(torch.sigmoid(x).permute(0,2,1).contiguous(), 2* self.patch_stride[1]) 
...
if __name__ == '__main__':
    import config
    import torch
    model = HTSAT_Swin_Transformer(num_classes=11, asc_num_classes=5, config=config)
    # print(model)
    inputs = torch.rand((1, 320000))
    out = model(inputs)
